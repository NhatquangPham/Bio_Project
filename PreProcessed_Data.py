...
# ── Ngưỡng lọc ───────────────────────────────────────────────────────────────
PREVALENCE_THRESHOLD = 0.10   # giữ genus xuất hiện ở ≥ 10% samples
MIN_COUNT_THRESHOLD  = 1      # OTU count tối thiểu để coi là "hiện diện"
CLR_PSEUDOCOUNT      = 0.5    # pseudocount thêm vào trước log (tránh log(0))
...
def split_meta_genus(df: pd.DataFrame):
    """
    Tách DataFrame thành (meta_df, genus_df).
    meta_df  : các cột Dataset, DiseaseState
    genus_df : các cột còn lại (raw counts hoặc đã chuẩn hóa)
    """
...
def save_with_meta(genus_df: pd.DataFrame,
                   ...
                   desc:     str):
    """
    Ghép meta vào trước genus rồi xuất CSV.
    Thứ tự cột: Dataset | DiseaseState | <genus>
    """
...
# ═════════════════════════════════════════════════════════════════════════════
# BƯỚC 1 – KIỂM TRA & XUẤT step1 (đã có sẵn, chuẩn hóa lại format)
# Mục đích: Xác nhận cấu trúc dữ liệu, thống kê sơ bộ, và xuất lại file step1_otu_with_labels.csv
# Lưu ý: Bước này không thực hiện bất kỳ lọc hay chuẩn hóa nào, chỉ đảm bảo rằng dữ liệu đã sẵn sàng cho bước tiếp theo.
# Lý do: Đảm bảo rằng dữ liệu đầu vào cho bước 2 đã có đầy đủ thông tin metadata và ở định dạng chuẩn, giúp tránh lỗi và nhầm lẫn trong các bước tiếp theo.
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# BƯỚC 2 – LỌC NHIỄU SINH HỌC
# Giữ lại các genus xuất hiện ở ≥ 10% samples (prevalence ≥ 10%)
# Lý do: Loại bỏ các genus quá hiếm, có thể là nhiễu sinh học hoặc lỗi kỹ thuật, giúp giảm sparsity và cải thiện hiệu suất ML.
# ═════════════════════════════════════════════════════════════════════════════
...
def step2_filter_noise(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── 2a: Tính prevalence (tỷ lệ mẫu có genus đó > MIN_COUNT_THRESHOLD) ────
    ...
    # ── 2b: Lọc thêm bất kỳ cột nào hoàn toàn = 0 (nếu còn) ─────────────────
...
# ═════════════════════════════════════════════════════════════════════════════
# BƯỚC 3a – CHUẨN HÓA TSS (Total Sum Scaling / Relative Abundance)
# Dùng cho phân tích trực quan (biểu đồ stacked bar, heatmap, v.v.)
# ═════════════════════════════════════════════════════════════════════════════
...
def step3a_tss_normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # TSS: chia từng hàng cho tổng của hàng đó
    tss_df = genus_df.div(row_totals, axis=0)
    tss_df = tss_df.fillna(0)   # xử lý mẫu có tổng = 0

    # Kiểm tra: mỗi hàng phải cộng lại ≈ 1 (ngoại trừ mẫu zero-total)
    ...
    # Thống kê: top 5 genus phong phú nhất
...
# ═════════════════════════════════════════════════════════════════════════════
# BƯỚC 3b – CHUẨN HÓA CLR (Centered Log-Ratio)
# Dùng cho AI/Machine Learning (giữ nguyên tính chất compositional, tránh double transformation)
# ═════════════════════════════════════════════════════════════════════════════

def step3b_clr_normalize(df: pd.DataFrame) -> pd.DataFrame:
    """
    CLR transformation (Aitchison 1986):
        1. Thêm pseudocount nhỏ (0.5) để tránh log(0)
        2. clr(x_i) = log(x_i / geometric_mean(x))
           tức là: log(x_i) - mean(log(x)) theo từng hàng

    Lý do CLR là tiêu chuẩn vàng cho microbiome:
    - Xử lý tính chất "compositional" (dữ liệu thành phần)
    - Đưa dữ liệu về không gian Euclidean thực
    - Cho phép dùng được các thuật toán ML thông thường
    """
    ...
    # Bước 1: thêm pseudocount
    X = genus_df.values.astype(float) + CLR_PSEUDOCOUNT

    # Bước 2: log transform
    log_X = np.log(X)

    # Bước 3: tính geometric mean theo từng hàng (= mean của log)
    gm_log = log_X.mean(axis=1, keepdims=True)   # shape (n_samples, 1)

    # Bước 4: CLR = log(x) - geometric_mean_log
    clr_X = log_X - gm_log
    ...
    # Kiểm tra: tổng mỗi hàng phải = 0 (tính chất của CLR)
...
def build_taxonomy_annotation(df: pd.DataFrame) -> pd.DataFrame:
    """
    Xây dựng bảng phân loại học chi tiết cho từng genus còn lại sau lọc.
    Dùng file-S3_core_genera.txt làm nguồn annotation (taxonomy lineage + nhãn health/disease).
    Bổ sung thêm thống kê từ dữ liệu thực tế.
    """
    ...
    # ── Đọc core_genera file để lấy full taxonomy lineage ────────────────────
    core_genera_path = BASE_DIR / "file-S3_core_genera.txt"

    lineage_map    = {}   # genus → full taxonomy string
    health_map     = {}   # genus → overall label (health/disease/mixed/'')
    ...
    # ── Xây dựng bảng annotation ─────────────────────────────────────────────
    rows = []
    for genus in sorted(genus_list):
        lineage = lineage_map.get(genus, "")

        # Parse từng cấp phân loại từ lineage string
        tax_levels = {
            "Kingdom" : "",
            "Phylum"  : "",
            "Class"   : "",
...
def main():
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # BƯỚC 3a: TSS (dùng dữ liệu đã lọc từ step2)
    # ─────────────────────────────────────────────────────────────────────────
    step3a_df = step3a_tss_normalize(step2_df)

    # ─────────────────────────────────────────────────────────────────────────
    # BƯỚC 3b: CLR (dùng dữ liệu đã lọc từ step2 — KHÔNG dùng TSS làm input)
    # Lưu ý quan trọng: CLR áp dụng trực tiếp lên raw counts (đã lọc),
    # KHÔNG áp dụng lên TSS để tránh double transformation.
    # ─────────────────────────────────────────────────────────────────────────
    step3b_df = step3b_clr_normalize(step2_df)

    # ─────────────────────────────────────────────────────────────────────────
    # TAXONOMY ANNOTATION (dùng danh sách genus sau lọc từ step2)
    # ─────────────────────────────────────────────────────────────────────────
